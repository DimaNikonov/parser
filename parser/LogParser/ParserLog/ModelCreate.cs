...
namespace ParserLog
{
    public class ModelsCreate<Loginfo> : ICreate<Loginfo> where Loginfo : class
    {
        ...
        private async Task RunTasksAsync(InfoAboutFile infoAboutFile)
        {
            ...
            Task task2 = GetTitleAsync(list, 2 * list.Count / countTread, 3 * list.Count / countTread);
            Task task3 = GetTitleAsync(list, 3 * list.Count / countTread, 4 * list.Count / countTread);
